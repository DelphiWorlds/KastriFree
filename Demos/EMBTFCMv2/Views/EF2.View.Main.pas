...
begin
  inherited;
  FPushNotifications := TPushNotifications.Create('EMBTFCMv2 Push Notifications');
...
begin
  ...
  TOSLog.d('Token Received: %s', [FPushNotifications.DeviceToken]);
  TOSLog.d('DeviceID: %s', [FPushNotifications.DeviceID]);
  if PushUDP <> nil then
    PushUDP.UpdateDeviceInfo(FPushNotifications.DeviceID, FPushNotifications.DeviceToken);
end;

initialization
  TOSLog.Tag := 'EMBTFCMv2';
